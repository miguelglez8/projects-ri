...
public class InvoiceAssembler {
	...
	public static InvoiceDALDto toInvoiceDALDto(InvoiceBLDto invoice) {
		InvoiceDALDto record = new InvoiceDALDto();
		
		record.id = invoice.id;
		record.version = invoice.version;
		record.amount = invoice.total;
		record.vat = invoice.vat;
		record.number = invoice.number;
		record.date = invoice.date;
		record.state = String.valueOf(invoice.state);
